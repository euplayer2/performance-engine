...
export default function App({ user, onSignOut }) {
  ...
  const saveEdit = async () => {
    ...
    setEditingId(null); setEditData(null); flash("✏️ Tarefa atualizada!");
  ...
  if (dataLoading) {
    return (
      <div style={{ minHeight: "100vh", background: "linear-gradient(165deg,#0D0D0F 0%,#1A1A2E 40%,#16213E 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit',sans-serif" }}>
  ...
  const PAGE_META = {
    tasks:     { icon: "📋", title: "Minhas Tarefas" },
    add:       { icon: "➕", title: "Nova Tarefa" },
    stats:     { icon: "📊", title: "Estatísticas" },
    customize: { icon: "🎨", title: "Personalizar" },
  };
  const meta = PAGE_META[tab] || PAGE_META.tasks;
  ...
  return (
    <div style={{
      ...
      {/* ═══════════════════ MAIN CONTENT ═══════════════════ */}
      <main style={{ marginLeft: 200, flex: 1, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: -0.5, margin: 0, color: "var(--text)" }}>{meta.title}</h1>
          </div>
                    ...
                    return (
                      <div key={t.id} style={{
                        ...
                        {!isEditing ? (
                          ...
                          <div>
                            <div style={{ fontSize: 10, color: "var(--blue)", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 }}>✏️ Editando tarefa</div>
              ...
              <div style={card()}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                  <input ref={inputRef} value={newTask.title} onChange={e => setNewTask({ ...newTask, title: e.target.value })}
                    ...
                    style={{ flex: 1, padding: "12px 14px", borderRadius: 10, border: "1px solid var(--border)", background: a("var(--text)", 4), color: "var(--text)", fontSize: 15, outline: "none", boxSizing: "border-box" }}
                ...
                {voiceGuide && (() => {
                  ...
                  return (
                    <div style={{ marginBottom: 20, padding: "12px 14px", borderRadius: 10, background: a("var(--accent)", 6), border: `1px solid ${a("var(--accent)", 12)}` }}>
          ...
          {tab === "stats" && (
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
                {[
                  { l: "Tarefas",    v: todayT.length,       icon: "📋", sub: null },
                  { l: "Concluídas", v: done.length,          icon: "✅", sub: null },
                  { l: "Estimado",   v: fmt(totalPlan),       icon: "📐", sub: "planejado" },
                  { l: "Tempo Real", v: fmtS2M(totalRealSec), icon: "⏱", sub: "cronômetro" },
                ].map(({ l, v, icon, sub }) => (
                  <div key={l} style={card({ padding: "14px 16px" })}>
                    <div style={{ fontSize: 10, color: "var(--textFaint)", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>{icon} {l}</div>
              ...
              {/* Eficiência (se houver concluídas) */}
              {done.length > 0 && (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div style={{ ...card({ textAlign: "center" }), background: a("var(--accent)", 6), borderColor: a("var(--accent)", 10) }}>
                  ...
                  <div style={{ fontSize: 32, fontWeight: 800, fontFamily: "'Space Mono', monospace", color: "var(--accent)" }}>{streak}</div>
                  <div style={{ fontSize: 12, color: "var(--textFaint)", marginTop: 4 }}>dias consecutivos</div>
                </div>
                <div style={card()}>
                  ...
                  <button onClick={resetAll} style={{ padding: "8px 16px", borderRadius: 8, border: "1px solid var(--border)", background: "transparent", color: "var(--textFaint)", fontSize: 12, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}>
                    🗑 Resetar tudo
                  </button>
                </div>
              </div>
